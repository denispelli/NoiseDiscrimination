...
T$letter_size = round(T$letter_size, digits = 2)
...
pp[[5]] <- ggplot(subset(T, T$noise_contrast==0.16 & T$noise_decay_radius==32 & T$TargetCross==0), aes(x=shiftedEccentricity, y=mean_Efficiency, color=factor(letter_size))) +
  ...
  # facet_grid(letter_size ~.)+
  noGrid+
  labs(title = "NoiseContrast 0.16, Inf Gaussian pink", x = "shiftedEccentricity(deg)", y = "Efficiency")+
  theme(text=element_text(size=32))

print(pp[[5]])

xbreaks = c(0.5, 1, 2, 3, 6, 9, 16, 32)
ybreaks = c(1e-05, 2e-05, 3e-05, 4e-05 ,5e-05, 6e-05, 5e-06)
ylimits = aes(ymax = mean_Neq + sd_Neq, ymin=mean_Neq - sd_Neq)
pp[[6]] <- ggplot(subset(T, T$noise_contrast==0.16 & T$noise_decay_radius==32 & T$TargetCross==0), aes(x=shiftedEccentricity, y=mean_Neq, color=factor(letter_size))) +
  geom_errorbar(ylimits, size = 2, width = .3)+
  geom_line(size = 2)+
  geom_point(size=12, alpha=0.6)+
  scale_y_log10(breaks=ybreaks)+
  ...
  # facet_grid(letter_size ~.)+
  noGrid+
  labs(title = "NoiseContrast 0.16, Inf Gaussian pink", x = "shiftedEccentricity(deg)", y = "Neq")+
  theme(text=element_text(size=32))
