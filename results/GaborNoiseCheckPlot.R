...
pp = list()
T <- read.csv("xiuyun_pretests.csv")
T<-T[1:14,]
T$noiseCheckDeg[is.na(T$noiseCheckDeg)] <- 0.802364
T$noiseCheckDeg = round(T$noiseCheckDeg, digits = 2)
...
ylimits = aes(ymax = mean_Efficiency + sd_Efficiency, ymin=mean_Efficiency - sd_Efficiency)
pp[[5]] <-   ggplot(subset(T, T$noise_contrast==0.16), aes(x=shiftedEccentricity, y=mean_Efficiency, color=targetKind, size=factor(noiseCheckDeg))) +
  ...
  # facet_grid(letter_size ~.)+
  noGrid+
  labs(title = "NoiseContrast 0.16, Gaussian White, gabor(global), letter(local)", x = "shiftedEccentricity(deg)", y = "Efficiency")+
  theme(text=element_text(size=32))
...
# ybreaks = c(1e-06, 1e-05, 1e-04, 1e-03, 1e-02)
ylimits = aes(ymax = mean_Neq + sd_Neq, ymin=mean_Neq - sd_Neq)
pp[[6]] <- ggplot(subset(T, T$noise_contrast==0.16), aes(x=shiftedEccentricity, y=mean_Neq, color=targetKind, size=factor(noiseCheckDeg))) +
  geom_errorbar(ylimits, size = 2, width = .3, alpha = 0.6)+
  geom_line(size = 2)+
  geom_point(alpha=0.6)+
  scale_y_log10(breaks=ybreaks)+
  scale_x_log10(breaks=xbreaks)+
  ...
  # facet_grid(letter_size ~.)+
  noGrid+
  labs(title = "NoiseContrast 0.16, Gaussian White, gabor(global), letter(local)", x = "shiftedEccentricity(deg)", y = "Neq")+
  theme(text=element_text(size=32))
print(pp[[6]])
