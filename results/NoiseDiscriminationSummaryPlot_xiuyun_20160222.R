...
T$noiseDecayRadiusOverTargetSize[T$noiseDecayRadiusOverTargetSize==0] = 0.1 # WITHOUT noice fix
...
# pp[[2]] <- ggplot(subset(T, T$targetKind=='gabor'), aes(x=targetSize, y=meanThreshold, color=factor(eccentricity), linetype=factor(noiseContrast) )) +
  ...
  ylimits = aes(ymax = meanThreshold + sdThreshold, ymin=meanThreshold - sdThreshold)
  pp[[2]] <- ggplot(rbind(subset(bakT, noiseDecayRadius>0 & noiseSpectrum == 'white' & noiseContrast == 0.16),subset(bakT, noiseContrast==0)), aes(x=noiseDecayRadiusOverTargetSize, y=meanThreshold, size=factor(targetSize), color=factor(shiftedEccentricity), shape=factor(noiseContrast))) +
