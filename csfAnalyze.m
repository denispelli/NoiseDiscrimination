...
for expt={'csfLettersStatic' 'csfGaborsStatic'}
    ...
    vars={'functionNames' 'conditionName' 'experiment' 'dataFilename' ...
        ...
        'pixPerCm' 'nearPointXYPix' 'trialsDesired' 'NUnits' 'isNoiseDynamic' 'noiseCheckFrames'};
    for iFile=1:length(matFiles) % One file per iteration.
        ...
        if readExperiment
            ...
            if ~isfield(d,'o') || d.o.trials<d.o.trialsDesired
                continue
            end
            ooo={d.o};
        end
    ...
    for oi=length(oo):-1:1
        if isempty(oo(oi).trials) || oo(oi).trials<oo(oi).trialsDesired
    ...
    if sum([oo.trials]<oo(1).trialsDesired)>0
        warning('Discarding %d threshold(s) with fewer than %d trials: %s',sum([oo.trials]<oo(1).trialsDesired),oo(oi).trialsDesired,s);
    end
    ...
    if averageAcrossObservers
        ...
        for conditionName=unique({oo.conditionName})
            for noiseSD=unique([oo.noiseSD])
                ...
                aa(ai).isNoiseDynamic=oo(find(ii,1)).isNoiseDynamic;
...
vars={'experiment' 'conditionName' ...
    ...
    'noiseType' 'noiseSD'  'noiseCheckDeg' ...
    'eccentricityXYDeg' 'viewingDistanceCm' 'eyes' ...
    'LBackground'  'dataFilename' 'isNoiseDynamic' 'noiseCheckFrames'};
t=struct2table(oo);
...
caption{3}=sprintf('%.1f cd/m^2, %.1f s, %s, isNoiseDynamic %d, noiseCheckFrames %d', ...
    oo(1).luminanceAtEye,oo(1).targetDurationSecs,oo(1).targetKind,oo(1).isNoiseDynamic,oo(1).noiseCheckFrames);
