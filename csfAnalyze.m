...
cd(dataFolder);
if readExperiment
    % Each file contains an experiment. One cell per block. One struct
    % array per block, with an element per condition and its threshold.
...
% array oo, with one element per condition and its threshold.
oo=[];
...
vars={'functionNames' 'conditionName' 'experiment' 'dataFilename' ...
   ...
   'pixPerCm' 'nearPointXYPix' 'trialsPerBlock'};
for iFile=1:length(matFiles) % One file per iteration.
   ...
   if readExperiment
       ...
       if ~contains(d.ooo{1}(1).functionNames,experiment)
           fprintf('Skipping function name: %s\n',d.ooo{1}(1).functionNames);
           continue
       end
       ooo=d.ooo;
   else
      if ~isfield(d,'o') || d.o.trials<10
         continue
      end
      ooo={d.o};
   end
...
if isempty(oo)
   fprintf('Couldn''t find any data.\n');
   return
end
...
[~,ii]=sort(oo.eccentricityXYDeg(1));
oo=oo(ii);
% Sort by sf.
