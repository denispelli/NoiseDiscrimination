...
temp1 = find(tabdata.noiseDecayRadius==inf & tabdata.noiseRadiusDeg==0); % hard edge
tabdata.noiseContrast(temp1) = 0;

temp1 = find(tabdata.noiseDecayRadius==0); % soft edge
tabdata.noiseContrast(temp1) = 0;

temp2 = find(tabdata.noiseContrast==0);
for ii = 1:length(temp2)
    if tabdata.noiseDecayRadius(temp2(ii))==inf % hard edge
        tabdata.noiseRadiusDeg(temp2(ii)) = 0;
    elseif tabdata.noiseRadiusDeg(temp2(ii)) >17 %soft edge
        tabdata.noiseDecayRadius(temp2(ii))==0;
    end;
end;
