...
%
% Under macOS, if the USB device is not available, ColorCal2 does nothing
% and returns the empty matrix [].
%
...
% 06.06.2017  Make compatible with Windows via serial port implementation
%             (Mario Kleiner).
%             This uses code derived from the Matlab sample code by CRS,
%             incorporated in a modified form with permission of CRS under
%             our usual MIT license.
% 08.01.2017  Under macOS, check that the USB device is available before
%             trying to open it. If not available, immediately return empty
%             matrix []. Thanks to Dee (Diederick C. Niehorster
%             dcnieho@gmail.com) for suggesting the test. (Denis Pelli)
%
...
if isempty(usbHandle) && isempty(portHandle)
   if ~IsWin
      ...
      % Is a CRS colorimeter available?
      available=0;
      d=PsychHID('Devices');
      for i=1:length(d)
