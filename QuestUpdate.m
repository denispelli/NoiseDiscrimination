...
% Copyright (c) 1996-2016 Denis Pelli
% 6/11/96 dgp Wrote it.
% 2/28/97 dgp Updated for Matlab 5: call round.
% 4/12/99 dgp Dropped support for Matlab 4.
% 4/30/99 dgp Give explanatory error message when intensity is out of 
%             bounds.
...
% 3/21/16 dgp Give explicit error when "reponse" argument is empty or
%             vector.
...
if isempty(q)
   error('"q" argument is empty.');
end
if length(q)>1
	error('"q" must be a simple struct, not an array.')
end
if isempty(intensity)
   error('"intensity" argument is empty.');
end
if length(intensity)~=1
   error('"intensity" must be a real number, not an array.');
end
if ~isreal(intensity)
    error('"intensity" %s must be real, not complex.',num2str(intensity));
end
if isempty(response)
   error('"response" argument is empty.');
end
if length(response)~=1
   error('"response" must be an integer, not an array.');
end
if ~(isreal(response) || islogical(response))
	error('"response" %g should be an integer or logical.',response);
end
if response<0 || response>=size(q.s2,1)
	error('"response" %g out of range 0 to %d',response,size(q.s2,1)-1);
end
if q.updatePdf
	...
	if ii(1)<1 || ii(end)>size(q.s2,2)
		if q.warnPdf
			...
			warning(['QuestUpdate: "intensity" %.3f out of range %.2f to %.2f. ' ...
            'Pdf will be inexact. Suggest that you increase "range" in '...
            'call to QuestCreate.'],intensity,low,high);
			warning(oldWarning);
		end
		if ii(1)<1
			ii=ii+1-ii(1);
