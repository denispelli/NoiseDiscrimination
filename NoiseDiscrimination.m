...
if 0
   %% ACCEPT ONLY KNOWN PARAMETERS
   initialFields=fieldnames(o);
   ...
   % we ignore it silently. We warn of unknown fields
   % because they might be typos for input fields.
   knownFields={'EnableCLUTMapping' 'desiredClutMargin' 'testBitDepth' ...
      'useFractionOfScreen' 'distanceCm' 'flipScreenHorizontally' 'observer'...
      'luminanceTransmission' 'trialsPerRun' 'runNumber' 'runsDesired' ...
      'speakInstructions' 'congratulateWhenDone' 'runAborted' 'quitNow' ...
      'targetKind' 'font' 'allowAnyFont' 'alphabet' 'printTargetBounds' ...
      'targetGaborPhaseDeg' 'targetGaborSpaceConstantCycles' ...
      'targetGaborCycles' 'targetGaborOrientationsDeg' 'targetGaborNames'...
      'targetModulates' 'task' 'thresholdParameter' 'alternatives' ...
      'tGuess' 'tGuessSd' 'pThreshold' 'beta' 'measureBeta' ...
      'eccentricityDeg' 'targetHeightDeg' 'minimumTargetHeightChecks'...
      'durationSec' 'useFlankers' 'flankerContrast' 'flankerSpacingDeg' ...
      'noiseSD' 'annularNoiseSD' 'noiseCheckDeg' 'noiseRadiusDeg' ...
      'noiseEnvelopeSpaceConstantDeg' 'noiseRaisedCosineEdgeThicknessDeg' ...
      'noiseSpectrum' 'showBlackAnnulus' 'blackAnnulusContrast' ...
      'blackAnnulusSmallRadiusDeg' 'blackAnnulusThicknessDeg' ...
      'annularNoiseBigRadiusDeg' 'annularNoiseSmallRadiusDeg' 'noiseType'...
      'noiseFrozenInTrial' 'noiseFrozenInRun' 'noiseFrozenInRunSeed' ...
      'targetCross' 'fixationCrossDeg' 'fixationCrossWeightDeg'...
      'fixationCrossBlankedNearTarget' ...
      'fixationCrossBlankedUntilSecAfterTarget' 'textSizeDeg' ...
      'saveSnapshot' 'snapshotLetterContrast' 'tSnapshot' 'cropSnapshot'...
      'snapshotCaptionTextSizeDeg' 'snapshotShowsFixationBefore'...
      'snapshotShowsFixationAfter' 'saveStimulus' 'gapFraction4afc'...
      'showCropMarks' 'showResponseNumbers' 'responseNumbersInCorners' ...
      'printSignalDuration' 'printCrossCorrelation' 'printLikelihood' ...
      'assessLinearity' 'assessContrast' 'assessLoadGamma' ...
      'assessLowLuminance' 'flipClick' 'isKbLegacy' 'assessGray'...
      'assessTargetLuminance' 'observerQuadratic' 'backgroundEntropyLevels'...
      'idealEOverNThreshold' 'screen' 'alphabetPlacement' ...
      'replicatePelli2006' 'isWin' 'movieFrameFlipSec' ...
      'useDynamicNoiseMovie' 'moviePreSec' 'moviePostSec' ...
      'likelyDurationSec' 'measuredDurationSec' 'printDurations' ...
      'newClutForEachImage' 'labelAlternatives' 'beginningTime' ...
   ...
   for condition=1:conditions
      ...
      for i=1:length(inputFields)
         if ismember(inputFields{i},initialFields)
            % Overwrite initial value.
            oo(condition).(inputFields{i})=oIn(condition).(inputFields{i});
         elseif ~ismember(inputFields{i},knownFields)
...
%% SCREEN PARAMETERS
% 3/23/17 moved to after reading o parameters. Untested in new location.
    ...
    if ~ismember(o.observer,algorithmicObservers) || streq(o.task,'identify')
        ...
        if exist('cal')
            ...
            if o.isWin
                ...
                % Otherwise (using OS other than Windows) we have two clut
                % entries that prduce the same gray. One (gray) is in the
                % middle of the CLUT and one is at entry 1 (gray1). The
                % benefit of having gray1==1 is that we get better blending
                % of letters written (as black=0) on that background.
                gray1 = 1;
                % Allow for margins
                ...
                cal.nFirst = gray1;
                cal.nLast = gray1+1;
                cal.clutMargin = 0; % IMPORTANT: must be zero.
                cal = LinearizeClut(cal);
            end % if o.isWin
...
%             Screen('LoadNormalizedGammaTable',window,cal.gamma,loadOnNextFlip);
            ...
            if o.assessGray
                ...
                ffprintf(ff,'Line %d: Gray index is %d (%.1f cd/m^2). Corner is %d.\n',...
                    MFileLineNr,gray,LuminanceOfIndex(cal,gray),pp(1));
            end
            if trial == 1
...
global window fixationLines fixationCrossWeightPix labelBounds location screenRect ...
    tTest idealT64 leftEdgeOfResponse textSize checks img cal ff
