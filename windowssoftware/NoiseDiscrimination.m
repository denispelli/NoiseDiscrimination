    ...
    if ~ismember(o.observer,{'ideal','brightnessSeeker','maximum'}) || streq(o.task,'identify')
        ...
        if o.flipScreenHorizontally
            ...
            if exist('cal')
                ...
                LMin=min(cal.old.L);
                LMax=max(cal.old.L);
                LMean=mean([LMin,LMax]);
                cal.LFirst=LMean;
                cal.LLast=LMean;
                % The -1 and +1 are a harmless hack. It appears that a one-entry
                % lookup table update doesn't load. Making it a 3-entry
                % lookup table seems to do the trick. i think it loads now.
                % I'm guessing that this is a problem in Screen
                % LoadNormalizedGammaTable but I'm not sure, so I haven't
                % reported it to the psychtoolbox forum. Denis June 2015
                gray1=1;
                cal.nFirst=gray1;
                cal.nLast=gray1;
...
Screen('FillRect',window,gray1);
        ...
        if exist('cal')
            ...
            % Create CLUT entry for gray background, at middle entry of our grayscale range.
            cal.LFirst=LMean;
            cal.LLast=LMean;
            cal.nFirst=gray;
            cal.nLast=gray;
            cal=LinearizeClut(cal);
            Screen('LoadNormalizedGammaTable',window,cal.gamma,1); % load at next flip
        end
        Screen('FillRect',window,gray1);
        Screen('FillRect',window,gray,o.stimulusRect);
        ...
        switch o.observer
            ...
            otherwise % human o.observer
                ...
                if newClutForEachImage
                    ...
                    % Compute clut for all possible images.
                    % Note: The gray screen is drawn with CLUT index n=1.
                    %
                    % Noise
