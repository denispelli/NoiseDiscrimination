...
if ~skipDataCollection && 1
   ...
   for oi=1:length(oo) % Edit this line to select the conditions you want to run now.
      ...
      %          o.conditionName,oOut.luminanceAtEye,oOut.luminanceFactor,oOut.filterTransmission);
      if isfield(oOut,'psych')
         fprintf(['<strong>%s: luminanceAtEye %.1f cd/m^2, '...
            'eccentricity %.1f %.1f deg, '...
            'duration %.2f s, cyclesPerDeg %.1f, '...
            'noiseSD %.2f, log N %.2f, '...
            'contrast %.3f</strong>\n'],...
            oOut.conditionName,oOut.luminanceAtEye,...
         ...
         oo(oi).luminanceAtEye=oOut.luminanceAtEye;
   ...
   %       'luminanceAtEye' 'eccentricityXYDeg' ...
   %       'targetDurationSec' 'targetCyclesPerDeg' ...
   %       'targetGaborCycles' 'targetHeightDeg' 'noiseSD' 'noiseCheckDeg'};
   vars={'experiment' 'condition' 'observer' 'trials' 'luminanceAtEye' 'eccentricityXYDeg' 'targetDurationSec' 'targetCyclesPerDeg' 'noiseSD' 'N' 'contrast'};
   if any(rows)
      disp(t(rows,vars)) % Print the oo list of conditions, with measured threshold.
   end
